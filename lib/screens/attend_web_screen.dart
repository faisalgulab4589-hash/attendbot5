...
class _AttendWebScreenState extends State<AttendWebScreen> {
  ...
  void _setup() {
    _wvc = WebViewController()
      ...
      ..setOnPlatformPermissionRequest((req) => req.grant()) // auto-allow location
      ...
      ..setNavigationDelegate(NavigationDelegate(
        onPageFinished: (url) async {
          ...
          // Override confirm/alert globally so no popup blocks us
        ...
        onWebResourceError: (e) {
          if (e.isForMainFrame == true) _setStatus('Error: ${e.description}');
        },
      ));
...
(function(){
  ...
  Bridge.postMessage('filled:email=' + !!ef + ',pass=' + !!pf + ',role=' + !!rs);
})();
''');
...
(function(){
  ...
  for (var i = 0; i < btns.length; i++) {
    ...
    if (t.includes('login') || t.includes('sign in') || t.includes('submit')) {
      btns[i].click();
      Bridge.postMessage('login-clicked:' + t);
      return;
    }
  }
  var f = document.querySelector('form');
  if (f) { f.submit(); Bridge.postMessage('form-submitted'); return; }
  Bridge.postMessage('login-btn-not-found');
})();
''');
  }

  // ─── Step 3: Click CHECK IN or CHECK OUT ───────────────────────────────
  Future<void> _clickAttendBtn() async {
    ...
    // Override confirm so "Are you sure?" is auto-accepted
    await _wvc.runJavaScript("window.confirm=function(){return true;};window.alert=function(){};");
...
(function(){
  ...
  var els = document.querySelectorAll(
    'button,a,input[type=button],input[type=submit],[role=button],[class*=check],[id*=check]'
  );
  for (var i = 0; i < els.length; i++) {
    var t = (els[i].textContent || els[i].value || els[i].innerText || '')
              .replace(/\\s+/g, ' ').trim().toLowerCase();
    if (t === '$kw1' || t === '$kw2' || t === '$kw3'
        || t.includes('$kw1') || t.includes('$kw2')) {
...
''');
  ...
  void _onMsg(String msg) async {
    ...
    if (msg.startsWith('DONE')) {
      ...
      // Wait 1.5s for server to process
      ...
      // Auto close after 1.5s — no user action needed
      await Future.delayed(const Duration(milliseconds: 1500));
      if (mounted) Navigator.of(context).pop(true);

    } else if (msg == 'FINAL_NOT_FOUND') {
      _setStatus('Button not found — please tap it manually');
    } else {
      // Intermediate messages (filled, login-clicked etc.) — just log
      _setStatus(msg.length > 60 ? msg.substring(0, 60) : msg);
    }
  }
        ...
        Container(
          ...
          child: SafeArea(bottom: false, child: Padding(
            ...
            child: Row(children: [
              ...
              // Close button — only show if done, otherwise no easy escape
              if (_done)
                IconButton(
