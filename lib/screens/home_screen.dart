...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _runNow(String mode) async {
    s.url      = urlCtrl.text.trim();
    s.email    = emailCtrl.text.trim();
    s.password = passCtrl.text;
    await PrefsService.save(s);
  ...
  void _toggleBot(bool on) async {
    ...
    s.password = passCtrl.text;
    await PrefsService.save(s);
    _snack(on ? 'Auto Bot ON ✓ — will attend at set times' : 'Auto Bot OFF',
      on ? const Color(0xFF2E7D32) : Colors.grey);
  }

  void _pickTime(bool isIn) async {
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(
                    s.botEnabled ? '● Auto attendance ON' : '○ Auto attendance OFF',
                    style: TextStyle(
                ...
                Transform.scale(scale: 0.85, child: CupertinoSwitch(
                  value: s.botEnabled,
              ...
              // Time pills
              Row(children: [
                _timePill('CHECK IN',  s.inTime,  const Color(0xFF2E7D32), true),
                const SizedBox(width: 10),
                _timePill('CHECK OUT', s.outTime, const Color(0xFFB71C1C), false),
              ]),
        ...
        SliverPadding(
          ...
          sliver: SliverList(delegate: SliverChildListDelegate([
            ...
            _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _fieldRow(Icons.language_rounded, 'Website URL',
                child: TextField(
                  controller: urlCtrl,
                  keyboardType: TextInputType.url,
                  style: const TextStyle(fontSize: 14),
                  decoration: _inputDeco('https://lms.gims.tech/'),
                )),
            ])),
            ...
            _card(Column(children: [
              _fieldRow(Icons.email_outlined, 'Email',
                child: TextField(
                  ...
                  style: const TextStyle(fontSize: 14),
                  decoration: _inputDeco('your@email.com'),
                )),
              const Divider(height: 1),
              _fieldRow(Icons.lock_outline_rounded, 'Password',
                child: TextField(
                  controller: passCtrl,
              ...
              Row(children: [
                ...
                const Text('Stored only on your device — never shared',
                  style: TextStyle(color: Colors.grey, fontSize: 10)),
              ]),
            ])),

            // ── Auto Time ──
            _secLabel('AUTO TIMING'),
            _card(Column(children: [
              ...
              Container(
                ...
                child: const Row(children: [
                  ...
                  Expanded(child: Text(
                    ...
                    'and press CHECK IN / CHECK OUT at the set times. No user action needed.',
            ...
            _card(Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Text('Works every day at set times',
                  style: TextStyle(color: Colors.grey, fontSize: 11)),
                const Text('Internet / flight mode handled automatically',
                  style: TextStyle(color: Colors.grey, fontSize: 11)),
              ])),
              CupertinoSwitch(
                value: s.botEnabled,
            ...
            Row(children: [
              Expanded(child: _bigBtn('✅  CHECK IN',  const Color(0xFF1B5E20), () => _runNow('checkin'))),
              const SizedBox(width: 10),
              Expanded(child: _bigBtn('🚪  CHECK OUT', const Color(0xFFB71C1C), () => _runNow('checkout'))),
            ]),

            // ── History ──
            const SizedBox(height: 20),
            _secLabel('ATTENDANCE HISTORY'),
            _card(s.history.isEmpty
              ? const Padding(padding: EdgeInsets.all(8),
                  child: Text('No records yet', style: TextStyle(color: Colors.grey)))
              : Column(children: s.history.reversed.take(10).toList().asMap().entries.map((e) {
  ...
  // ── helpers ──────────────────────────────────────────────────────────────
  Widget _timePill(String lbl, String time, Color c, bool isIn) =>
    Expanded(child: GestureDetector(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 2),
          Row(children: [
            Text(time, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold)),
            const SizedBox(width: 4),
  ...
  Widget _fieldRow(IconData icon, String label, {required Widget child}) =>
...
// ── AM/PM Time Picker ─────────────────────────────────────────────────────
class _AmPmTimePicker extends StatefulWidget {
  final String title;
  final Color color;
  final int hour, min;
  final bool isPm;
  final Future<void> Function(int, int, bool) onSave;
  const _AmPmTimePicker({required this.title, required this.color,
    required this.hour, required this.min, required this.isPm, required this.onSave});
  @override State<_AmPmTimePicker> createState() => _AmPmTimePickerState();
}

class _AmPmTimePickerState extends State<_AmPmTimePicker> {
  ...
  Widget _wheel(List<String> items, int init, ValueChanged<int> onChange) =>
    SizedBox(width: 52, child: ListWheelScrollView.useDelegate(
      ...
      childDelegate: ListWheelChildBuilderDelegate(
        ...
        builder: (_, i) => Center(child: Text(items[i],
          style: TextStyle(fontSize: i==init?22:15,
            fontWeight: i==init?FontWeight.bold:FontWeight.normal,
            color: i==init?widget.color:Colors.grey))),
      ),
    ));

  Widget _amPmBtn(String lbl, bool selected) => GestureDetector(
